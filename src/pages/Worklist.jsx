...
const Worklist = () => {
  ...
  return (
    <div className='home-page tasks bg-cyan-500 w-screen min-h-screen flex justify-center min-w-[450px] overflow-x-hidden overflow-y-auto'>
          ...
          {Array.isArray(task) && task.length > 0 ? (
            task.map((e, idx) => (
              <div
                ...
                key={e._id+idx}
              >
                <TaskCard todo={e} worklist={data.worklist} setSubWork={setSubWork} setEditIdx={setEditIdx}/>
              </div>
            ))
          ) : (
      ...
      <div className={`absolute flex top-0 z-100 bg-[#001aff] ${(subWork)?"h-full w-full":'h-0 w-0'}`}>
        {(

          (subWork===1)?<NewTaskForm subWork={subWork} setSubWork={setSubWork}/>
          :(subWork===2)&&<EditTask subWork={subWork} setSubWork={setSubWork} task={task[editIdx]}/>
        )}

...
const Worklist = () => {
  ...
  return (
    <div className='home-page w-screen min-h-screen flex justify-center bg-gradient-to-br from-cyan-400 to-blue-500 text-gray-900 overflow-x-hidden overflow-y-auto'>
      ...
      <div className='mt-12 w-full flex flex-col items-center'>
        ...
        >
          {Array.isArray(task) && task.length > 0 ? (
            task.map((e, idx) => (
              <div
                className='task-box w-[90%] sm:w-[80%] md:w-[60%] bg-white/90 backdrop-blur-md shadow-lg hover:shadow-xl transition-all my-3 rounded-xl border border-gray-200 p-4'
                key={e._id + idx}
              >
                <TaskCard
                  todo={e}
      ...
      >
        <svg fill={themeStyles.primary} viewBox="0 0 155 155" className='h-8 w-8'>
      ...
      {subWork !== 0 && (
        <div className='absolute inset-0 w-full h-full backdrop-blur-sm flex justify-center items-center z-50 mt-20'>
          {subWork === 1 && <NewTaskForm subWork={subWork} setSubWork={setSubWork} />}
          {subWork === 2 && <EditTask setSubWork={setSubWork} todo={task[editIdx]} editIdx={editIdx} />}
        </div>
      )}
    </div>
  );

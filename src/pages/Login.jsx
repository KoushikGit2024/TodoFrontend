...
const Login = () => {
  ...
  return (
    <div className="h-full  w-full flex items-center justify-center bg-gradient-to-br from-[#e3f2fd] via-[#fef6e4] to-[#fff5eb] px-4">
      <motion.div
        initial={{ opacity: 0, y: 40 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.7, ease: "easeOut" }}
        className="w-full max-w-md my-10 bg-white/80 backdrop-blur-lg shadow-2xl border border-[#EBCB90]/50 rounded-2xl p-8"
      >
        <motion.h2
          initial={{ opacity: 0, scale: 0.8 }}
          animate={{ opacity: 1, scale: 1 }}
          transition={{ delay: 0.2 }}
          className="text-4xl font-bold text-center text-[#3396D3] mb-6"
        >
          Login
        </motion.h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <motion.input
            ...
            placeholder="Username or Email"
            autoComplete="username"
            className="w-full h-12 px-3 text-[#3396D3] bg-[#f8f8f8] focus:bg-white border-2 border-[#EBCB90] rounded-md outline-none caret-[#3396D3]"
            required
          />

          <motion.div
            className="flex items-center bg-[#f8f8f8] focus-within:bg-white h-12 border-2 border-[#EBCB90] rounded-md"
            whileFocus={{ scale: 1.02 }}
          >
            <input
              type={showPw ? "text" : "password"}
            ...
            <button
              type="button"
              onClick={() => setShowPw((prev) => !prev)}
              className="h-6 w-6 mr-3 text-[#3396D3] hover:scale-110 transition"
            >
              {showPw ? "🙈" : "👁️"}
            </button>
          </motion.div>
          ...
          {message.text && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              className={`w-full text-center py-2 rounded-md text-white font-medium ${
                message.type === "success" ? "bg-green-500" : "bg-red-500"
              }`}
            >
              {message.text}
          ...
          <div className="flex justify-between items-center mt-4">
            ...
            <motion.button
              ...
              type="button"
              onClick={handleCancel}
              className="px-6 py-2 rounded-md bg-white border border-[#d4d4d4] text-[#333] hover:bg-[#f5f5f5]"
            >
              Cancel
            </motion.button>
          </div>

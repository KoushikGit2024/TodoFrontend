...
const EditTask = ({ setSubWork, todo, editIdx }) => {
  const { data } = useContext(DataContext);
  const { task, setTask } = useContext(TaskContext);
  const navigate = useNavigate();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (listType !== todo.listType) taskData.listType = listType;
  ...
  return (
    <div className="bg-[#ececdb] w-full h-full flex items-center justify-center p-4 mt-40">
      <div className="bg-[#ece3d9] shadow-2xl w-full max-w-2xl rounded-2xl overflow-hidden border-2 border-[#0edbff] p-6">
        <form className="flex flex-col w-full" onSubmit={handleSubmit}>
          ...
          {/* <h2 className="text-3xl font-bold border-b-4 border-[#EBCB90] pb-2 text-[#5a4a3f]">
            ✏️ Edit Task
          </h2> */}

...
//         <nav className=' w-[95%] bg-[#ebe3d9] h-16 mt-4 rounded-full overflow-hidden p-4 border-2 min-w-[540px] flex flex-row items-center justify-between shadow-xl z-2' style={{color:"#3396D3"}}>
...
//                         </div>
//                     </Link>
//                     <div className="search bg-[#e4e4e4] h-10 rounded-full border-[2px] p-1 flex flex-row items-center justify-around w-[80%]" style={{borderColor:"#EBCB90"}}>
//                         <input type="text" className='outline-none rounded-l-full pl-3 bg-transparent h-8 w-full bg-transparenut' placeholder='Search Task' style={{caret:"#3396D3"}}
...
//                                 <path d="M27.414,24.586l-5.077-5.077C23.386,17.928,24,16.035,24,14c0-5.514-4.486-10-10-10S4,8.486,4,14  s4.486,10,10,10c2.035,0,3.928-0.614,5.509-1.663l5.077,5.077c0.78,0.781,2.048,0.781,2.828,0  C28.195,26.633,28.195,25.367,27.414,24.586z M7,14c0-3.86,3.14-7,7-7s7,3.14,7,7s-3.14,7-7,7S7,17.86,7,14z" id="XMLID_223_" fill={"#3396D3"}/>
//                             </svg>
//                         </span>
//                     </div>    
//                 </div>
...
//             </div>
            ...
            //         <div className='bg-wheel h-12 w-12 absolute rounded-full' style={{background:`conic-gradient(#3396D3 0% 25%,#FFF0CE 25% 50%,"#EBCB90" 50% 75%,"#EEEEEE" 75% 100%)`}}></div>
...
//                 className="absolute right-0 mt-3 w-50 bg-[#ebe3d9] border-2 rounded-lg flex flex-col border-[#3396D3] drop-shadow-2xl p-5 overflow-hidden"
//               >
...
const Navbar = () => {
  ...
  const searchHandler = async (text) => {
    let res = await axios.get(`/api/user/tasks?search=${text}`, { withCredentials: true })
          ...
          <AnimatePresence>
            {dropDown && (
              <motion.div 
                ...
                exit={{ opacity: 0, y: -10, height: 0 }}
                transition={{ duration: 0.3 }}
                className="absolute right-0 mt-70 w-50 bg-[#ebe3d9] border-2 rounded-lg flex flex-col border-[#3396D3] drop-shadow-2xl p-5 overflow-hidden"
              >
                <Link className='menu-item' to='/user/signup'>Create New Account</Link>                
                <Link className='menu-item' to='/user/profile'>Manage Profile</Link>
                <Link className='menu-item' to='/user'>Analytics</Link>
                <Link className='menu-item' to='/user/tasks/worklist'>Tasks</Link>
                <Link className='menu-item' to='/user/login'>Add Another Account</Link>
                <Link className='menu-item' to=''>Log Out</Link>
              </motion.div>
